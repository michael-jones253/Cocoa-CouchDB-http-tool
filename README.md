...
This is my first go at Objective-C and I dived in to quickly get the feel for the language and also get the CouchDB project functioning. I suspect I am not doing everything using best practises, but I have started to read Google's Objective-C Style guide and refactoring my code accordingly.
...
What I am reading about Objective-C exceptions is that exceptions are only intended for programmer coding errors (e.g array out of bounds) and are otherwise discouraged.
...
I think one of the reasons Apple chose their error return instead of an exception model is that it encourages the programmer to included localised string content describing the error which can be displayed on internationalised GUIs. However, I remember dealing with this problem in a C# WPF GUI, my solution was to produce an internationlised exception class.
